...
#(python) da implementação de um Sistema de Recomendação, no contexto de
#e-commerce levando em conta o histórico de compras do usuário, utilizando
#Redes Neurais e implementado em Python.
...
product_categories = {
    ...
    "Eletrodomésticos": ["Geladeira", "Micro-ondas", "Máquina de lavar", "Ar condicionado"],
...
def encode_purchase_history(user_purchase_history, product_categories):
    all_products = [product for products in product_categories.values() for
    product in products]
...
def forward_propagation(X, W1, b1, W2, b2):
    Z1 = [sum(X[j] * W1[j][i] for j in range(len(X))) + b1[i] for i in range(len(b1))]
    A1 = relu(Z1)
    Z2 = [sum(A1[j] * W2[j][i] for j in range(len(A1))) + b2[i] for i in range(len(b2))]
    A2 = sigmoid(Z2)
    return A2
...
def recommend_products(user_purchase_history, product_categories):
    encoded_history = encode_purchase_history(user_purchase_history, product_categories)
    ...
    recommendations = [all_products[i] for i in range(len(all_products)) if
recommendations_encoded[i] > 0.5 and all_products[i] not in
user_purchase_history]

    return recommendations

# Obter recomendações de produtos
recommendations = recommend_products(user_purchase_history,
product_categories)
